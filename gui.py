...
class AudioPlayer:
    ...
    def _get_duration(self, file_path: str) -> float:
        """Return audio duration in seconds using pydub (imageio-ffmpeg backed)."""
        try:
    ...
    def seek(self, position: float) -> None:
        with self._lock:
            ...
            if not self._file:
                return
            # WAV files do not support timed seek in pygame; reload from start.
            if self._file.lower().endswith(".wav"):
...
class ProjectCard(tk.Frame):
    """Reusable sidebar card representing a generated audio file."""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.lbl_title = tk.Label(self.info_frame, text=self.display_name,
                                   font=("", 10, "bold"), bg=COLOR_CARD,
                                   fg=COLOR_TEXT_MAIN, anchor="w")
        self.lbl_title.pack(anchor="w")

        self.lbl_subtitle = tk.Label(self.info_frame, text=self.mod_time,
                                      font=("", 10), bg=COLOR_CARD,
                                      fg=COLOR_TEXT_DIM, anchor="w")
        self.lbl_subtitle.pack(anchor="w")

        self.btn_toggle = tk.Button(
            self.top_frame, text="▶", font=("", 10), width=3, height=1,
    ...
    def update_state(self, is_active, is_playing, duration, position):
        ...
        if is_active != self.is_active:
            ...
            if is_active:
                ...
                for w in (self.inner, self.top_frame, self.info_frame, self.bottom_frame):
                    w.configure(bg="#fff8f4")
...
class GramoVoice:
    """Main application controller for GramoVoice Studio."""
    ...
    def __init__(self, root: tk.Tk, engine: TTSEngine) -> None:
        ...
        # Resolve and create output directory; fall back to ~/GramoVoice on failure.
        self.output_dir = self.settings.get("output_dir", str(Path.home() / "GramoVoice"))
    ...
    def _init_output_dir(self) -> None:
        try:
            ...
            fallback = str(Path.home() / "GramoVoice")
            logger.warning(
                f"No write permission for '{self.output_dir}'; using '{fallback}' instead."
            )
    ...
    def _load_logo(self, width: int = 240):
        try:
    ...
    def _setup_window(self) -> None:
        ...
        try:
            if sys.platform == "win32":
                # Windows: iconbitmap accepts .ico files natively.
                ico_path = Path(__file__).parent / "assets" / "ico.ico"
                if ico_path.exists():
                    self.root.iconbitmap(str(ico_path))
            else:
                # Linux / macOS: use PNG via PhotoImage.
    ...
    def _show_splash(self) -> None:
        ...
        if self.splash_logo:
            ...
            tk.Label(container, text="GRAMOVOICE", font=("", 36, "bold"),
                     bg=COLOR_BG, fg=COLOR_ACCENT).pack(pady=(0, 10))

        self.lbl_splash_status = tk.Label(container, text="INITIALIZING...",
                                           font=("", 12), bg=COLOR_BG, fg=COLOR_TEXT_DIM)
        self.lbl_splash_status.pack(pady=(0, 10))
        self.lbl_splash_perc = tk.Label(container, text="0%",
                                         font=("", 10), bg=COLOR_BG, fg=COLOR_TEXT_DIM)
    ...
    def _initialization_flow(self) -> None:
        self.root.after(0, lambda: self.splash_progress.configure(mode="indeterminate"))
        self.root.after(0, self.splash_progress.start)

        def on_status(text, color=None):
            self.root.after(0, lambda: self.lbl_splash_status.configure(text=text.upper()))
            if isinstance(color, (int, float)):
                self.root.after(0, lambda: self.lbl_splash_perc.configure(text=f"{int(color*100)}%"))

        def on_progress(v):
            if isinstance(v, (int, float)) and v >= 0:
                self.root.after(0, lambda: (
                    self.splash_progress.configure(mode="determinate", value=v * 100),
                    self.lbl_splash_perc.configure(text=f"{int(v * 100)}%"),
                ))
    ...
    def _show_error(self) -> None:
        for w in self.splash_frame.winfo_children():
            w.destroy()
        tk.Label(self.splash_frame, text="ERROR LOADING ENGINE",
                 font=("", 20), bg=COLOR_BG, fg=COLOR_CANCEL).pack(expand=True)
    ...
    def _build_ui(self) -> None:
        ...
        if self.logo_img:
            ...
            tk.Label(inner_side, text="GramoVoice", font=("", 24, "bold"),
                     bg=COLOR_SIDEBAR, fg=COLOR_ACCENT).pack(anchor="w", pady=(0, 10))

        tk.Frame(inner_side, height=1, bg=COLOR_BORDER).pack(fill="x", pady=10)

        tk.Label(inner_side, text="VOICE MODEL", font=("", 10, "bold"),
                 bg=COLOR_SIDEBAR, fg=COLOR_ACCENT).pack(anchor="w")
        self.voice_var = tk.StringVar()
        self.drop_voices = ttk.Combobox(inner_side, textvariable=self.voice_var, state="readonly")
        self.drop_voices.pack(fill="x", pady=(4, 10))
        self.drop_voices.bind("<<ComboboxSelected>>", lambda e: self._save_voice_speed())

        tk.Label(inner_side, text="OUTPUT QUALITY", font=("", 10, "bold"),
                 bg=COLOR_SIDEBAR, fg=COLOR_ACCENT).pack(anchor="w")
        self._quality_options = {
            "Compacto VBR  (~0.45 MB/min)": "compact",
            "Balanceado VBR (~0.60 MB/min)": "balanced",
            "Padrão 128k   (~0.96 MB/min)": "standard",
            "Alta 192k      (~1.44 MB/min)": "high",
        }
        self._quality_labels = list(self._quality_options.keys())
        self._quality_keys = list(self._quality_options.values())
        self.quality_var = tk.StringVar()
        self.drop_quality = ttk.Combobox(inner_side, textvariable=self.quality_var,
                                          values=self._quality_labels, state="readonly")
        self.drop_quality.pack(fill="x", pady=(4, 15))
        self.drop_quality.bind("<<ComboboxSelected>>", lambda e: self._save_voice_speed())

        saved_quality = self.settings.get("output_quality", "balanced")
        try:
            idx = self._quality_keys.index(saved_quality)
        except ValueError:
            idx = 1  # default: balanced
        self.drop_quality.current(idx)

        speed_header = tk.Frame(inner_side, bg=COLOR_SIDEBAR)
        speed_header.pack(fill="x", pady=(4, 0))
        tk.Label(speed_header, text="SPEED", font=("", 10, "bold"),
                 bg=COLOR_SIDEBAR, fg=COLOR_ACCENT).pack(side="left")
        self.lbl_speed_val = tk.Label(speed_header, text="1.0x", font=("", 10, "bold"),
        ...
        tk.Label(inner_side, text="PROJECT HISTORY", font=("", 9, "bold"),
                 bg=COLOR_SIDEBAR, fg=COLOR_TEXT_DIM).pack(anchor="w")
        ...
        self._bind_scrollwheel(hist_cont, self.canvas)
        ...
        self.txt_project_name.bind(
            "<Control-a>",
            lambda e: (e.widget.select_range(0, "end"), e.widget.icursor("end"), "break")[2],
        )
        self.txt_project_name.bind(
            "<Control-A>",
            lambda e: (e.widget.select_range(0, "end"), e.widget.icursor("end"), "break")[2],
        ...
        self.txt_input = tk.Text(
            editor, font=("", 14), height=12, bg=COLOR_CARD, fg=COLOR_TEXT_MAIN,
            bd=0, padx=10, pady=10, wrap="word", insertbackground=COLOR_ACCENT,
        )
        self.txt_input.pack(fill="both", expand=True)
        self.txt_input.bind("<KeyRelease>", lambda e: self._update_char_counter())
        self.txt_input.bind("<Control-a>", lambda e: self.txt_input.tag_add("sel", "1.0", "end") or "break")
        self.txt_input.bind("<Control-A>", lambda e: self.txt_input.tag_add("sel", "1.0", "end") or "break")

        self.lbl_char_count = tk.Label(content, text="0 chars", font=("", 10),
        ...
        self.lbl_progress = tk.Label(prog_frame, text="0%", font=("", 9),
                                      bg=COLOR_BG, fg=COLOR_TEXT_DIM)
        ...
        self.btn_gen = tk.Button(
            btn_frame, text="GENERATE AUDIO", font=("", 12, "bold"),
            bg=COLOR_ACCENT, fg="white", bd=0, height=2, cursor="hand2",
            command=self._handle_generate,
        )
        self.btn_gen.pack(side="left", fill="x", expand=True)
        tk.Button(
            btn_frame, text="📂 FOLDER", font=("", 10, "bold"),
            bg=COLOR_CARD, fg=COLOR_ACCENT, bd=0, width=10, height=2,
            command=self._open_output_folder,
        ).pack(side="right", padx=5)

        tk.Label(content, text="ENGINE LOGS", font=("", 9, "bold"),
                 bg=COLOR_BG, fg=COLOR_TEXT_DIM).pack(anchor="w", pady=(10, 5))
        ...
        self.log_console = tk.Text(
            log_frame, height=6, bg=COLOR_LOG_BG, fg="#1e8e3e",
            font=("Courier", 11), bd=0, padx=10, pady=10, state="disabled", wrap="word",
        )
        ...
        lbl_user = tk.Label(footer, text="unusual_zeru", font=("", 9),
        ...
        lbl_aliens = tk.Label(footer, text="aliens 👽🖖", font=("", 9),
                               bg=COLOR_BG, fg=COLOR_ACCENT, cursor="hand2")
        ...
        lbl_repo = tk.Label(footer, text="gramovoice", font=("", 9),
                             bg=COLOR_BG, fg=COLOR_ACCENT, cursor="hand2")
    ...
    def _bind_scrollwheel(self, widget: tk.Widget, canvas: tk.Canvas) -> None:
        """Bind mouse-wheel scroll cross-platform (Windows/macOS vs Linux)."""
        if sys.platform in ("win32", "darwin"):
            def _on_wheel(e):
                canvas.yview_scroll(-1 if e.delta > 0 else 1, "units")
            widget.bind("<Enter>", lambda e: self.root.bind_all("<MouseWheel>", _on_wheel))
    ...
    def _on_speed_change(self, v) -> None:
    ...
    def _save_voice_speed(self) -> None:
        self.settings["model"] = self.drop_voices.get()
        self.settings["speed"] = float(self.sld_speed.get())
        selected_label = self.drop_quality.get()
        self.settings["output_quality"] = self._quality_options.get(selected_label, "balanced")
        ...
        try:
            ...
            if force or (now - self.last_history_poll > 3):
                ...
                try:
                    ...
                    logger.warning(f"Cannot list output directory: {e}")
                    return

                files = sorted(
                    [f for f in all_files if f.endswith(".mp3") or f.endswith(".wav")],
                ...
                if files != self.last_history_files:
                    ...
                    for f in files:
                        ...
                        card = ProjectCard(
                            self.history_scroll, f, full, self._toggle_play, self._seek_audio
                        )
            ...
            for path, card in self.cards.items():
                card.update_state(
                    path == active,
                    playing if path == active else False,
    ...
    def _toggle_play(self, filename: str) -> None:
    ...
    def _seek_audio(self, pos: float) -> None:
        if self.player:
    ...
    def _handle_generate(self) -> None:
        ...
        def run():
            try:
                quality_label = self.drop_quality.get()
                mp3_quality = self._quality_options.get(quality_label, "balanced")
                ...
                success = self.engine.synthesize(
                    ...
                    chunk_callback=lambda msg: logger.info(msg),
                )
                ...
                if success:
                    ...
                    messagebox.showerror("Error", "Synthesis failed! Check the engine logs.")
            finally:
                self.is_generating = False
                self.root.after(
                    0,
                    lambda: self.btn_gen.configure(state="normal", text="GENERATE AUDIO", bg=COLOR_ACCENT),
                )

        threading.Thread(target=run, daemon=True).start()


def main(skip_engine: bool = False) -> None:
